...
//call help big modal
  $('.help-header-link a,#menu-Futer-Dopomogti-ukr li:eq(0) a,#menu-Futer-Dopomogti-rus  li:eq(0) a,#menu-Futer-Dopomogti-en li:eq(0) a').click(function(e){
    e.preventDefault();
  ...
  setTimeout(function(){
    $( ".section-news-hover-wrapper" ).each(function( index ) {
        if( $( this ).find('.vc_gitem-zone-img').attr('src').indexOf('vc_gitem_image.png') >= 0 ){
          $( this ).addClass('no-background-image');
          $( this ).attr('style','background-image: linear-gradient(224deg, #f3ae43 0%, #e78b48 100%)!important');
        }
        else{
  ...
  $(".woocommerce-custom-category-filter .category-filter-item a[href*='"+path+"']").addClass("active-filter");
  $(".woocommerce-custom-category-filter .category-filter-item a.active-filter").parent().addClass("active");
   

  $('.woocommerce-custom-product-filter .filter-cart-price-wrapper .woocommerce-custom-price-filter a').click(function(e){
    e.preventDefault();
...
var message_amount_free, message_amount_not_number, message_fio_free, message_fio_not_text, message_mail_required, message_phone_required, message_phone_only_number, message_oferta_required;
if (lang === 'uk'){
