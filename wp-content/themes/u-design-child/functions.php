...
add_action('woocommerce_checkout_create_order', 'createOrder_ru_address', 10, 2);
...
function createOrder_ru_address($order){
    if (pll_current_language() === 'ru') {
        ...
        $npCity = $wpdb->get_row("SELECT description_ru FROM wc_ukr_shipping_np_cities WHERE area_ref = '" . esc_attr($_POST[WC_UKR_SHIPPING_NP_SHIPPING_NAME . '_city']) . "'", ARRAY_A);
        if ($npCity) {
        ...
        $npWarehouse = $wpdb->get_row("SELECT description_ru FROM wc_ukr_shipping_np_warehouses WHERE city_ref = '" . esc_attr($_POST[WC_UKR_SHIPPING_NP_SHIPPING_NAME . '_warehouse']) . "'", ARRAY_A);
        if ($npWarehouse) {
