...
class NHP_Options{
	...
	*/
	function _enqueue(){
		...
		wp_register_style(
				'nhp-opts-css',
                CHILD_DIR.'/inc/options/options/css/options.css',
				array('farbtastic'),
				time(),
				'all'
			);
		wp_register_style(
			...
			apply_filters('nhp-opts-ui-theme', CHILD_DIR.'/inc/options/options/css/jquery-ui-aristo/aristo.css'),
			'',
			time(),
			'all'
		);
		...
		wp_enqueue_script(
			'nhp-opts-js',
            CHILD_DIR.'/inc/options/options/js/options.js',
			array('jquery'),
			time(),
			true
		);
